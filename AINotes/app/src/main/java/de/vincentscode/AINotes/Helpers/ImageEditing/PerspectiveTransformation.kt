...
class PerspectiveTransformation {
    ...
    private fun sortCorners(corners: MatOfPoint2f): MatOfPoint2f {
        ...
        val topPoints: MutableList<Point> =
            ArrayList()
        val bottomPoints: MutableList<Point> =
            ArrayList()
        for (point in points) {
            if (point.y < center.y) {
                topPoints.add(point)
        ...
        val topLeft =
            if (topPoints[0].x > topPoints[1].x) topPoints[1] else topPoints[0]
        val topRight =
            if (topPoints[0].x > topPoints[1].x) topPoints[0] else topPoints[1]
        val bottomLeft =
